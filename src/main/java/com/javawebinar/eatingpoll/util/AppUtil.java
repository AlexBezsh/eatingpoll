...
public class AppUtil {
    ...
    public static Long parseId(String id) {
        ...
        if (num <= 0) throw new BadRequestException("Received id is less than one");
        return num;
    }
    ...
    public static User checkEntity(User user, String name, String email, String password, Role role) {
        checkName(name);
        if (password == null || password.isBlank() || password.length() < 5 || password.length() > 20) throw new BadRequestException("Password should be between 5 and 20 characters");
    ...
    public static String checkName(String name) {
        ...
        if (name.length() < 2 || name.length() > 60) throw new BadRequestException("Name should be between 2 and 60 characters");
        return name;
    }
}
