...
<body>
    ...
    <c:forEach items="${restaurants}" var="restaurant">
        ...
        <h4>${restaurant.name}
            ...
            <a href="${pageContext.request.contextPath}/dish/create?restaurantId=${restaurant.id}&userId=${user.id}">(add dish)</a>
        </h4>
        <p>Set New Name:
        <form:form action="restaurant/update?userId=${user.id}" method="post" modelAttribute="restaurant">
            <table border="0" cellpadding="5">
                <tr>
                    <td><form:hidden path="id" value="${restaurant.id}"/></td>
                    <td><form:input path="name"/></td>
                    <td colspan="2"><input type="submit" value="Save"></td>
                </tr>
            </table>
        ...
        <p>Number of Votes: ${restaurant.votesCount} <a href="${pageContext.request.contextPath}/vote?restaurantId=${restaurant.id}&userId=${user.id}">Choose</a></p>
